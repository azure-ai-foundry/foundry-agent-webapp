 ...
 */
export type SseEventType = 'threadId' | 'chunk' | 'usage' | 'done' | 'error';
...
export interface SseThreadIdEvent extends SseEvent {
  type: 'threadId';
  data: {
    threadId: string;
  };
...
export function parseSseLine(line: string): SseEvent | null {
  ...
  try {
    ...
    // Backend sends flat structure: {type: "threadId", threadId: "xyz"}
    // Map to expected structure: {type: "threadId", data: parsed}
