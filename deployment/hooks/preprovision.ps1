...
try {
    ...
    if (-not [string]::IsNullOrWhiteSpace($existingEndpoint) -and 
        ...
        if (-not [string]::IsNullOrWhiteSpace($existingAgentId)) {
            Write-Host "  Agent ID: $existingAgentId" -ForegroundColor Gray
        } else {
            # Try to discover agent even with pre-configured endpoint
            Write-Host "Discovering agents in project..." -ForegroundColor Cyan
            $token = (az account get-access-token --resource 'https://ai.azure.com' 2>$null | ConvertFrom-Json).accessToken
            
            if ($token) {
                try {
                    $response = curl --request GET --url "$existingEndpoint/assistants?api-version=2025-05-01" `
                    ...
                    if ($agents -and $agents.Count -gt 0) {
                        if ($agents.Count -eq 1) {
                            Write-Host "[OK] Found 1 agent: $($agents[0].name) ($($agents[0].id))" -ForegroundColor Green
                            azd env set AI_AGENT_ID $agents[0].id
                        } else {
                            Write-Host "Found $($agents.Count) agents, using first: $($agents[0].name)" -ForegroundColor Yellow
                            azd env set AI_AGENT_ID $agents[0].id
                        }
                    }
                } catch {
...
Option 2 - Manually configure (if resource is in different subscription):
  ...
  azd env set AI_AGENT_ID <agent-id>
...
"@
        ...
        if ([string]::IsNullOrWhiteSpace($aiAgentId)) {
            ...
            if ($token) {
                try {
                    ...
                    $response = curl --request GET --url "$aiEndpoint/assistants?api-version=2025-05-01" `
                        -H "Authorization: Bearer $token" `
                        -H "Content-Type: application/json" `
                        --silent 2>$null
                    ...
                    if ($agents -and $agents.Count -gt 0) {
                        if ($agents.Count -eq 1) {
                            ...
                            Write-Host "[OK] Found 1 agent: $($selectedAgent.name) ($($selectedAgent.id))" -ForegroundColor Green
                            $aiAgentId = $selectedAgent.id
                            azd env set AI_AGENT_ID $aiAgentId
                        } else {
                            Write-Host "Found $($agents.Count) agents:" -ForegroundColor Yellow
                            for ($i = 0; $i -lt [Math]::Min($agents.Count, 5); $i++) {
                                $agent = $agents[$i]
                                Write-Host "  [$($i+1)] $($agent.name) - $($agent.id)" -ForegroundColor Gray
                            }
                            if ($agents.Count -gt 5) {
                                Write-Host "  ... and $($agents.Count - 5) more" -ForegroundColor Gray
                            }
                            Write-Host ""
                            Write-Host "Using first agent: $($agents[0].name)" -ForegroundColor Yellow
                            Write-Host "To use a different agent, run: azd env set AI_AGENT_ID <agent-id>" -ForegroundColor Gray
        ...
        # Final check for agent ID
        if ([string]::IsNullOrWhiteSpace($aiAgentId)) {
            Write-Host ""
            Write-Host "[!]  Agent ID not configured" -ForegroundColor Yellow
            Write-Host "You need to specify an agent ID to use with this application." -ForegroundColor Yellow
            Write-Host ""
            Write-Host "To set your agent ID:" -ForegroundColor Cyan
            ...
            Write-Host "  4. Copy the Agent ID from the agent's setup page" -ForegroundColor Gray
            Write-Host "  5. Run: azd env set AI_AGENT_ID <agent-id>" -ForegroundColor Gray
            Write-Host "  6. Run: azd up" -ForegroundColor Gray
            Write-Host ""
            Write-Error "AI_AGENT_ID is required. Please configure it and run 'azd up' again."
            exit 1
        } else {
            Write-Host "  Agent ID: $aiAgentId" -ForegroundColor Gray
        }
    }
