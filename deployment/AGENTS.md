...
$agentEndpoint = "https://$resourceName.cognitiveservices.azure.com"
$agents = az rest --method GET --url "$agentEndpoint/agents/v1.0/agents" | ConvertFrom-Json
...
azd env set AI_FOUNDRY_AGENT_ENDPOINT $agentEndpoint
azd env set AI_FOUNDRY_AGENT_ID $agentId
...
AI_FOUNDRY_AGENT_ENDPOINT=$agentEndpoint
AI_FOUNDRY_AGENT_ID=$agentId
...
**Pattern**: Updates Entra app redirect URIs, then calls shared `Build-AndDeployContainer.ps1` module
...
$scriptPath = Join-Path (Split-Path $PSScriptRoot -Parent) "scripts\Build-AndDeployContainer.ps1"
...
`deployment/scripts/Build-AndDeployContainer.ps1`
