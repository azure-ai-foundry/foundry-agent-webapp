...
3. **postprovision** → Updates redirect URIs + calls `Build-AndDeployContainer.ps1` to build/deploy container

**Deployment logic**: Shared `Build-AndDeployContainer` module (DRY) uses local Docker if available, ACR cloud build otherwise.
...
| `AI_FOUNDRY_AGENT_ENDPOINT not configured` | Run `azd provision` to re-discover AI Foundry resources |
